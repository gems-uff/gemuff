...
//#endif
        ...
        DCTHash* DCTHash::GenerateHash(VIMUFF::ImagePtr image){
            ...
            VIMUFF::Image* grayScaleT = image->getGrayResizedThumb();

            // 2 step: compute the dct
            VIMUFF::Image* dctImage = grayScaleT->dct();
            ...
            VIMUFF::Image* dctCropped = dctImage->subImage(1, 1, 8, 8);

            // 4 step: calculate the mean
            float mean = (float)dctCropped->mean();
            const float* data = reinterpret_cast<const float*>(dctCropped->getData());
        ...
        MarrHildretchHash *MarrHildretchHash::GenerateHash(VIMUFF::ImagePtr image){
            ...
            memset(mhHash->hash, 0, MH_HASH_SIZE * sizeof(uint8_t));

            VIMUFF::ImagePtr img(image->toGray());
            img.reset(img->blur(1.0f, 1.0f));
            img.reset(img->resize(512, 512));
            img.reset(img->equalize());
            img.reset(img->convertTo(CV_32F));
            ...
            img.reset(img->correlateNormalized(width, height, kernel));
            float _blockDataSum[30 * 30];
            for (int rindex=0;rindex < 30;rindex++){
                for (int cindex=0;cindex < 30;cindex++){
                	VIMUFF::ImagePtr subsec(img->subImage(cindex * 16, rindex * 16, 16 - 1, 16 - 1));
                	_blockDataSum[rindex * 30 + cindex] = subsec->sum();
                }
            }

            VIMUFF::ImagePtr _blocks;
            _blocks.reset(new VIMUFF::Image(30, 30, CV_32F, _blockDataSum));

            int hash_index;
            int nb_ones = 0, nb_zeros = 0;
            int bit_index = 0;
            unsigned char hashbyte = 0;
            for (int rindex=0;rindex < 30-2;rindex+=4){
                for (int cindex=0;cindex < 30-2;cindex+=4){
                    VIMUFF::ImagePtr subsec(_blocks->subImage(cindex, rindex, 2, 2));
        ...
        std::string MarrHildretchHash::toString(){
        	...
        	return ss.str();
        	/*
            char *res = new char[MH_HASH_SIZE*2+1];
            memset(res, 0, sizeof(char) * MH_HASH_SIZE * 2 + 1);

            for (size_t k = 0; k < MH_HASH_SIZE; k++)
                sprintf(&res[k*2], "%02x", hash[k]);

            res[MH_HASH_SIZE*2] = '\0';
            std::string _result = std::string(res);
            free(res);
            return _result;*/
